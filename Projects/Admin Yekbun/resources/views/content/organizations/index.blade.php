          ...
          <tr>
              ...
              <div class="d-flex justify-content-start align-items-center user-name">
                <div class="avatar-wrapper">
                  <div class="avatar me-2">
                    <img src="{{ asset('storage/' . $organization->image) }}" alt="Avatar" class="rounded-circle">
                  </div>
                </div>
            ...
            <td>
              <div class="form-check form-switch mb-2">
                <input class="form-check-input closetogglebtn" type="checkbox" id="flexSwitchCheckChecked " @if ($organization->status) checked @endif>
              </div>
            </td>
            <!-- OPTIONS -->
            <td>
              <div>
                ...
                <!-- Delete -->
                <form action="{{ route('organization.destroy', $organization->_id) }}" onsubmit="confirmAction(event, () => event.target.submit())" method="post" class="d-inline">
                  @method('DELETE')
                  @csrf
              ...
              --}}
              <div class="modal fade x-modal" id="editModal{{ $organization->_id }}" tabindex="-1" aria-labelledby="editModalLabel{{ $organization->_id }}" aria-hidden="true">
                <div class="modal-dialog modal-md">
                  <div class="modal-content">
                    <div class="modal-header bg-custom-grey pb-2">
                      <div class="d-flex w-100">
                        <div class="bg-white p-2 rounded-3 mx-auto">
                          <h5 class="modal-title fs-3" id="editModalLabel{{ $organization->_id }}">Edit Organization</h5> 
                        </div>
                      </div>
                    ...
                    <div class="modal-footer d-flex justify-content-center">
                      <button type="submit" class="btn btn-label-secondary d-flex gap-2" form="editForm{{ $organization->_id }}">Update  <img src="{{asset('assets/img/icons/donations/send.png')}}" alt="Send Icon"></button>
                    </div>
                  </div>
                </div>
              </div>
  ...
  <div class="modal fade x-modal" id="createModal" tabindex="-1" aria-labelledby="createModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-md">
      <div class="modal-content">
        ...
        <div class="modal-footer d-flex justify-content-center">
          <button type="submit" class="btn btn-label-secondary d-flex gap-2" form="createForm">Create <img src="{{asset('assets/img/icons/donations/send.png')}}" alt="Send Icon"></button>
        </div>
      </div>
    </div>
  </div>
...
<script>
  $(document).ready(function() {
    $('.logo-preview-div').on('click', function() {
        $('#logo_img').click();
    });
    ...
    $('#logo_img').on('change', function(event) {
        ...
        if (file) {
            ...
            const imgElement = $('<img>', { src: e.target.result, alt: 'organization logo', class: 'w-100 h-100 object-fit-cover rounded-circle' });
