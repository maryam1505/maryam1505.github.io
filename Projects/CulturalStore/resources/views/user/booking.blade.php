            ...
            <div class="container-fluid">
                    ...
                    <div class="card"> @if (session('success'))
                        ...
                        <div class="card-header">
                            ...
                            <button type="button" class="btn btn-primary btn-sm mb-2" data-toggle="modal"
                                data-target="#basicModal">Add Schedule</button>
                            <div class="modal fade" id="basicModal" tabindex="-1" role="dialog"
                                aria-labelledby="basicModalLabel" aria-hidden="true">
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="basicModalLabel">Add Schedule </h5>
                                        ...
                                        <form action="{{ route('admin.newschedule') }}" method="POST"
                                            ...
                                            <div class="modal-footer">
                                                <button type="submit"
                                                    class="btn btn-sm btn-primary">{{ isset($user) ? 'Update Schedule' : 'Add Schedule' }}</button>
                                ...
                                <table class="table table-responsive-md">
                                    <thead>
                                        <tr>
                                            ...
                                            <!-- Changed column to Engine Number -->
                                            <th></th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach($schedules as $schedule)
                                        <tr id="scheduleRow{{ $schedule->id }}">
                                            <td>{{ $schedule->id }}</td>
                                            <td>{{ $schedule->name }}</td> <!-- Display Name -->
                                            <td>{{ $schedule->scheduling_status }}</td>
                                            <!-- Display Scheduling Status -->
                                            <td>{{ $schedule->engine_number }}</td> <!-- Display Engine Number -->

                                            <td>
                                                <div class="d-flex">
                                                    <!-- Edit button -->
                                                    <a href="#"
                                                        class="btn btn-primary shadow btn-xs sharp mr-1 edit-company"
                                                        data-toggle="modal"
                                                    ...
                                                    <form id="deleteForm{{ $schedule->id }}"
                                                        action="{{ route('admin.deleteschedule', ['id' => $schedule->id]) }}"
                                                        method="post">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="button"
                                                    ...
                                                    <form
                                                        action="{{ route('admin.deactivatecompany', ['id' => $schedule->id]) }}"
                                                        method="post">
                                                        @csrf
                                                        @method('PUT')
                                                        <button type="submit"
                                                    ...
                                                    @else
                                                    <form
                                                        action="{{ route('admin.activatecompany', ['id' => $schedule->id]) }}"
                                                        method="post">
                                                        @csrf
                                                        @method('PUT')
                                                        <button type="submit"
                                        ...
                                        <div class="modal fade" id="updateModal{{ $schedule->id }}" tabindex="-1"
                                            role="dialog" aria-labelledby="updateModalLabel{{ $schedule->id }}"
                                            aria-hidden="true">
                                            <div class="modal-dialog" role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title"
                                                            id="updateModalLabel{{ $schedule->id }}">
                                                            Update
                                                            Company</h5>
                                                    ...
                                                    <div class="modal-body">
                                                        ...
                                                        <form id="updateCompanyForm{{ $schedule->id }}"
                                                            action="{{ route('admin.updatecompany', ['id' => $schedule->id]) }}"
                                                            ...
                                                            <div class="form-group">
                                                                ...
                                                                <input type="file" name="image"
                                                                    ...
                                                                    id="imageInput{{ $schedule->id }}">
                                                                <label>Preview</label><br>

                                                                <div
                                                                    style="position: relative; width: 150; height: 150px; border: 1px solid #ccc; overflow: hidden;">
                                                                    <!-- Adjust width and height as needed -->
                                                                    <img id="imagePreview{{ $schedule->id }}"
                                                                        src="{{ asset('storage/' . $schedule['image']) }}"
                                                                        alt="Image Preview"
                                                        ...
                                                        $(document).ready(function() {
                                                            $('#updateModal{{ $schedule->id }}').on(
                                                                'show.bs.modal',
                                                                function() {
                                                                    $('#imageInput{{ $schedule->id }}').on(
                                                                        'change',
                                                                        function(event) {
                                                                            var file = event.target
                                                                                .files[0];
                                                                            var imagePreview = $(
                                                                                '#imagePreview{{ $schedule->id }}'
                                                                            )[0];
                ...
                function deleteSchedule(scheduleId) {
                    if (confirm('Are you sure you want to delete this schedule?')) {
                        axios.delete('/admin/deleteschedule/' + scheduleId)
                            .then(function(response) {
                                if (response.status === 200) {
                                    document.getElementById('scheduleRow' + scheduleId).remove();
