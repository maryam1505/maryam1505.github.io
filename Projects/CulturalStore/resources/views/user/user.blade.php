        ...
        ***********************************-->
        <div class="nav-header">
            <a href="/admin/dashboard" class="brand-logo">
                ...
                <img src="{{ asset('storage/downloads/appicon.png') }}" width="40px" height="40px" alt="Profile Picture"
                    class="logo-img">
                <!-- Change the text in the second SVG to "Booking" -->
                <svg class="brand-title" width="145" height="22" viewBox="0 0 74 22" fill="none"
                    xmlns="http://www.w3.org/2000/svg">
                    <text x="0" y="16" font-family="Arial" font-size="16" fill="black">Mispa Motor</text>
                </svg>
            </a>
                    ...
                    <div class="col-lg-12">
                        ...
                        <div class="card-header">
                            ...
                            <button type="button" class="btn btn-primary btn-sm mb-2" data-toggle="modal"
                                data-target="#basicModal">Add User</button>
...
<script>
    ...
    <div class="form-group">
        ...
        <input type="text" class="form-control" id="phone_number" name="phone_number" value="{{ old('phone_number', isset($user) ? $user->phone_number : '') }}">
        <span id="phone_numberError" class="text-danger"></span>
    </div>
    <div class="form-group">
    <label for="picture" class="mr-3">New Picture:</label>
    <div class="d-flex align-items-center">
        <input type="file" name="picture" class="form-control-file mr-3" id="pictureInput">
        <div style="width: 200px; height: 200px; position: relative;">
            <img id="picturePreview" src="#" alt="Picture Preview" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; object-fit: cover;">
        </div>
    </div>
...
<script>
      document.getElementById('username').addEventListener('input', function() {
        ...
        axios.post('/check-username', { username: username })
    ...
    document.getElementById('pictureInput').addEventListener('change', function(event) {
        ...
        reader.onload = function(e) {
            picturePreview.src = e.target.result; // Set the image source to the file's data URL
        };
...
<script>
    document.getElementById('userForm').addEventListener('submit', function(event) {
        ...
        axios.post('/check-email', { email: email })
            .then(function(response) {
                var emailExists = response.data.exists;
...
<script>
    function deleteUser(userId) {
        if (confirm('Are you sure you want to delete this user?')) {
            axios.delete('{{ route('admin.delete', ['id' => $user->id]) }}')
                .then(function (response) {
...
<div id="status-{{ $user->id }}">
    ...
    <form action="{{ route('user.inactive', ['id' => $user->id]) }}" method="post">
        @csrf
    ...
    @endif
                                        ...
                                        <div class="modal fade" id="updateModal{{ $user->id }}" tabindex="-1"
                                            role="dialog" aria-labelledby="updateModalLabel{{ $user->id }}"
                                            aria-hidden="true">
                                            <div class="modal-dialog" role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header">
